...
import { SEAT_PRICES, BUNDLE_PRICES, formatCents } from '@domo/shared';

const FEATURES = [
  { title: 'Proactive AI butler', desc: 'Domo anticipates needs — oil changes, tax season, date nights — before you ask.' },
  ...
  { title: 'Voice, Jarvis-style', desc: 'Talk to Domo on your phone, tablet, or countertop display.' },
];
  ...
  return (
    <main className="min-h-screen bg-background text-textPrimary">
      <header className="flex items-center justify-between px-6 py-6 md:px-16">
        <span className="text-2xl font-bold tracking-tight">Domo</span>
      ...
      <section className="px-6 pb-24 pt-16 text-center md:px-16">
        ...
        <p className="mx-auto mt-6 max-w-xl text-lg text-textSecondary">
          Domo runs the household stuff — chores, schedules, meals, maintenance, money, and more — so you don't have to
      ...
      <footer className="border-t border-border px-6 py-8 text-center text-xs text-textSecondary md:px-16">
        © {new Date().getFullYear()} Domo. Built for families.
      </footer>
    </main>
  );
}
